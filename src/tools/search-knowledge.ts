...
// internalKnowledgeSearch is the RAG tool. It queries the Upstash Vector index for
...
export function makeInternalKnowledgeSearch(env: VectorEnv) {
  return tool({
    description: `Search the internal knowledge base for reference material on systems, processes, and topics the user might ask you to draw. Use this BEFORE drawing when the request touches a specific technical system, protocol, organizational structure, or process where precise details matter. The corpus contains short reference docs the model may not have memorized accurately.

Example: internalKnowledgeSearch({ query: "OAuth 2.0 authorization code flow with PKCE" })`,
